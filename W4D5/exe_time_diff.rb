...
def largest_continuous_sub_sum(list)
    ...
    while counter < list.length
        ...
        elsif curr_sum + list[counter] > curr_sum && valleys[-1] != counter - 1
    ...
    valleys.map do |valley|
        debugger
        val_total = list.take(valley).sum + list[valley] + list.drop(valley+1).sum
        if val_total > list.take(valley).sum && val_total > list.drop(valley+1).sum
    ...
    max_sum > list.sum ? max_sum : list.sum 
end
